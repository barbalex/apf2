...
CREATE TABLE apflora.infoflora20250407original(
...
CREATE INDEX ON apflora.infoflora20250407original USING btree(sisf_id);

CREATE INDEX ON apflora.infoflora20250407original USING btree(tax_id_intern);

CREATE INDEX ON apflora.infoflora20250407original USING btree(obs_id);

-- 2 import into apflora.infoflora20250407original
...
CREATE TABLE apflora.infoflora20250407beob(
...
CREATE INDEX ON apflora.infoflora20250407beob USING btree(obs_id);

CREATE INDEX ON apflora.infoflora20250407beob USING btree(already_imported);

-- 4 insert importdata into temp beob table
INSERT INTO apflora.infoflora20250407beob(external_id, obs_id, id_field, datum, autor, data, art_id, art_id_original, changed_by, geom_point, quelle)
SELECT
...
FROM
  apflora.infoflora20250407original ROW;

-- 393
--
-- 5 mark apflora kontrollen with is_apflora_ek = TRUE
-- nope, jordan says none
...
SELECT
  *
FROM
  apflora.infoflora20250407beob
WHERE
  id IN (
    SELECT
      info.id
    FROM
      apflora.infoflora20250407beob info
...
FROM
  ...
  INNER JOIN apflora.infoflora20250407beob new ON previous.obs_id = new.obs_id
