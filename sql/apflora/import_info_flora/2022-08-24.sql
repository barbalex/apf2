...
CREATE TABLE apflora.infoflora20220824original (
  GUID text,
  interpretation_note text,
  no_isfs integer,
  tax_id_intern integer, -- renamed by info flora to taxon_id this time!!!
  taxon text,
  doubt_status text, -- renamed by info flora this time
...
CREATE INDEX ON apflora.infoflora20220824original USING btree (no_isfs);

CREATE INDEX ON apflora.infoflora20220824original USING btree (tax_id_intern);
...
CREATE INDEX ON apflora.infoflora20220824beob USING btree (obs_id);

CREATE INDEX ON apflora.infoflora20220824beob USING btree (already_imported);

-- 4 insert importdata into temp beob table
INSERT INTO apflora.infoflora20220824beob (guid, obs_id, id_field, datum, autor, data, art_id, art_id_original, changed_by, geom_point, quelle)
SELECT
  uuid_or_null (guid),
  obs_id,
  'obs_id',
