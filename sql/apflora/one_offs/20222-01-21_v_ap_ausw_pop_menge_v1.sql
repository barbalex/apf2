...
with massnjahre AS (
  ...
  FROM
    ...
    INNER JOIN apflora.tpopkontrzaehl_einheit_werte ze0 ON ze0.id = ekze.zaehleinheit_id ON ekze.ap_id = ap.id
  ...
  WHERE
    ...
    AND massn.zieleinheit_einheit = ze0.code
...
zaehljahre AS (
  ...
  FROM
    ...
    INNER JOIN apflora.tpopkontrzaehl_einheit_werte ze2 ON ze2.id = ekze2.zaehleinheit_id ON ekze2.ap_id = ap2.id
      ...
      AND ap2.year = pop2.year ON pop2.id = tpop.pop_id
      AND pop2.year = tpop.year ON tpop.id = kontr.tpop_id
