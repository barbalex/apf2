-- Migration: rename 'Zwischenbeurteilung' to 'Kontrolle' in apflora.tpopkontr_typ_werte
-- As tpopkontr.typ is defined with ON UPDATE CASCADE, updating the werte table
...
WHERE
  ...
  AND kontr.jahr <> date_part('year', CURRENT_DATE) -- keine Kontrollen aus dem aktuellen Jahr - die wurden ev. noch nicht verifiziert
  AND tpop.bekannt_seit IS NOT NULL -- nur wenn erfasst ist, seit wann die TPop bekannt ist
  AND (tpop.status IN (100, 101) -- die Teilpopulation ist ursprünglich
    OR (kontr.jahr - tpop.bekannt_seit) > 5) -- oder bei Ansiedlungen: die Art war mindestens 5 Jahre vorhanden
...
-- Step 3: Recreate function apflora.q_tpop_ohne_tpopber
...
-- Step 4: Recreate function apflora.q_pop_ohne_popber
