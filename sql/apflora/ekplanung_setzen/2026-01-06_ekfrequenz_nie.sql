...
FROM
  apflora.tpop tpop
  -- BEWARE: this is run at start of year. Change if already running in december!
  INNER JOIN apflora.tpop_history tpophist ON tpop.id = tpophist.id 
    AND tpophist.year = date_part('year', CURRENT_DATE) -2
  INNER JOIN apflora.pop_status_werte status_werte_hist ON tpophist.status = status_werte_hist.code
...
WHERE tpop_id IN (
    ...
    FROM
      tpop_id_to_unplan)
  -- BEWARE: this is run at start of year. Change if already running in december!
  AND jahr > date_part('year', CURRENT_DATE) -1;
