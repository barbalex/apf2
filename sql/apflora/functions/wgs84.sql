...
CREATE OR REPLACE FUNCTION wgs84_lat(apflora.tpop)
  RETURNS float8 AS
$func$
    select ST_Y(geom_point) from apflora.tpop where apflora.tpop.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
-- drop function if exists wgs84_long(apflora.tpop);
CREATE OR REPLACE FUNCTION wgs84_long(apflora.tpop)
  RETURNS float8 AS
$func$
    select ST_X(geom_point) from apflora.tpop where apflora.tpop.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
...
CREATE OR REPLACE FUNCTION apflora.tpop_wgs84lat(tpop apflora.tpop) RETURNS float8 AS $$
  select ST_Y(tpop.geom_point) where is_valid_coordinates(tpop.geom_point) = true
$$ LANGUAGE sql STABLE;
...
CREATE OR REPLACE FUNCTION apflora.tpop_wgs84long(tpop apflora.tpop) RETURNS float8 AS $$
  select ST_X(tpop.geom_point) where is_valid_coordinates(tpop.geom_point) = true
$$ LANGUAGE sql STABLE;
...
CREATE OR REPLACE FUNCTION wgs84_lat(apflora.pop)
  RETURNS float8 AS
$func$
    select ST_Y(geom_point) from apflora.pop where apflora.pop.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
-- drop function if exists wgs84_long(apflora.pop);
CREATE OR REPLACE FUNCTION wgs84_long(apflora.pop)
  RETURNS float8 AS
$func$
    select ST_X(geom_point) from apflora.pop where apflora.pop.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
...
CREATE OR REPLACE FUNCTION apflora.pop_wgs84lat(pop apflora.pop) RETURNS float8 AS $$
  select ST_Y(pop.geom_point) where is_valid_coordinates(pop.geom_point) = true
$$ LANGUAGE sql STABLE;
...
CREATE OR REPLACE FUNCTION apflora.pop_wgs84long(pop apflora.pop) RETURNS float8 AS $$
  select ST_X(pop.geom_point) where is_valid_coordinates(pop.geom_point) = true
$$ LANGUAGE sql STABLE;
...
CREATE OR REPLACE FUNCTION wgs84_lat(apflora.beob)
  RETURNS float8 AS
$func$
    select ST_Y(geom_point) from apflora.beob where apflora.beob.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
-- drop function if exists wgs84_long(apflora.beob);
CREATE OR REPLACE FUNCTION wgs84_long(apflora.beob)
  RETURNS float8 AS
$func$
    select ST_X(geom_point) from apflora.beob where apflora.beob.id = $1.id and is_valid_coordinates(geom_point) = true
$func$ LANGUAGE SQL STABLE;
...
CREATE OR REPLACE FUNCTION apflora.beob_wgs84lat(beob apflora.beob) RETURNS float8 AS $$
  select ST_Y(beob.geom_point) where is_valid_coordinates(beob.geom_point) = true
$$ LANGUAGE sql STABLE;
...
CREATE OR REPLACE FUNCTION apflora.beob_wgs84long(beob apflora.beob) RETURNS float8 AS $$
  select ST_X(beob.geom_point) where is_valid_coordinates(beob.geom_point) = true
$$ LANGUAGE sql STABLE;