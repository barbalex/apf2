...
FROM
  crosstab ($$
    SELECT
      tpop_id, jahr, zaehleinheit, anzahl FROM ( WITH nr_of_kontr AS (
          SELECT
            apflora.tpop.id, count(apflora.tpopkontr.id) AS anzahl FROM apflora.tpop
  LEFT JOIN apflora.tpopkontr ON apflora.tpopkontr.tpop_id = apflora.tpop.id
  WHERE
    apflora.tpop.apber_relevant IS TRUE
    AND apflora.tpopkontr.apber_nicht_relevant IS NOT TRUE GROUP BY apflora.tpop.id
), letzte_ansiedlungen AS ( SELECT DISTINCT ON (tpop1.id)
    tpop1.id AS tpop_id, massn1.id AS massn_id FROM apflora.tpopmassn massn1
  ...
  ORDER BY tpop1.id, massn1.jahr DESC, massn1.datum DESC)
  -- 1. get zieleinheit from massnahmen counts
  SELECT
    * FROM ( SELECT DISTINCT ON (tpop2.id)
      tpop2.id AS tpop_id, massn0.jahr, massn0zieleinheitwerte.text AS zaehleinheit, massn0.zieleinheit_anzahl AS anzahl FROM apflora.tpopmassn massn0
      INNER JOIN apflora.tpopkontrzaehl_einheit_werte massn0zieleinheitwerte ON massn0zieleinheitwerte.code = massn0.zieleinheit_einheit
      INNER JOIN letzte_ansiedlungen ON letzte_ansiedlungen.massn_id = massn0.id
        AND letzte_ansiedlungen.tpop_id = massn0.tpop_id
      INNER JOIN apflora.tpop tpop2
      INNER JOIN nr_of_kontr ON nr_of_kontr.id = tpop2.id ON tpop2.id = massn0.tpop_id
    WHERE
      tpop2.apber_relevant IS TRUE
      AND massn0.jahr IS NOT NULL
      AND tpop2.status IN (200, 201)
      AND nr_of_kontr.anzahl = 0
      AND massn0.zieleinheit_anzahl IS NOT NULL ORDER BY tpop2.id, massn0.jahr DESC, massn0.datum DESC) AS triebe
  ...
  SELECT
    * FROM ( SELECT DISTINCT ON (tpop2.id)
    ...
    WHERE
      ...
      AND tpop2.status IN (200, 201)
      AND nr_of_kontr.anzahl = 0
      AND massn2.anz_triebe IS NOT NULL ORDER BY tpop2.id, massn2.jahr DESC, massn2.datum DESC) AS triebe
  UNION
  ...
  SELECT
    * FROM ( SELECT DISTINCT ON (tpop3.id)
    ...
    WHERE
      ...
      AND nr_of_kontr.anzahl = 0
      AND massn3.anz_pflanzen IS NOT NULL ORDER BY tpop3.id, massn3.jahr DESC, massn3.datum DESC) AS pflanzen
  UNION
  ...
  SELECT
    * FROM ( SELECT DISTINCT ON (tpop4.id)
    ...
    WHERE
      ...
      AND nr_of_kontr.anzahl = 0
      AND massn4.anz_pflanzstellen IS NOT NULL ORDER BY tpop4.id, massn4.jahr DESC, massn4.datum DESC) AS pflanzstellen
    -- 3. get all einheits from tpopkontr counts
  UNION
  SELECT
    * FROM ( SELECT DISTINCT ON (tpop5.id, apflora.tpopkontrzaehl_einheit_werte.text)
    ...
    WHERE
      ...
      AND kontr5.id = (
        ...
        WHERE
          ...
          AND kontr6.tpop_id = tpop5.id ORDER BY kontr6.jahr DESC, kontr6.datum DESC LIMIT 1)
  ORDER BY tpop5.id, apflora.tpopkontrzaehl_einheit_werte.text, kontr5.jahr DESC, kontr5.datum DESC) AS others) AS tbl ORDER BY 1, 2, 3 $$, $$
  SELECT
    unnest('{Pflanzen total, Pflanzen (ohne Jungpflanzen), Triebe total, Triebe Beweidung, Keimlinge, davon Rosetten, Jungpflanzen, Blätter, davon blühende Pflanzen, davon blühende Triebe, Blüten, Fertile Pflanzen, fruchtende Triebe, Blütenstände, Fruchtstände, Gruppen, Deckung (%), Pflanzen/5m2, Triebe in 30 m2, Triebe/50m2, Triebe Mähfläche, Fläche (m2), Pflanzstellen, Stellen, andere Zaehleinheit, Art ist vorhanden}'::text[]) $$) AS anzahl ("tpop_id" uuid,
    "jahr" integer,
    "Pflanzen total" integer,
...
tpop_letzte_menge AS (
  ...
  FROM
    ...
    LEFT JOIN massnjahre AS mj ON mj.tpop_id = tpop3.id
      AND mj.jahr = (
        SELECT
          max(jahr)
        FROM
          massnjahre
      WHERE
        massnjahre.jahr <= tpop3.year
        AND massnjahre.tpop_id = tpop3.id)
      LEFT JOIN zaehljahre AS zj ON zj.tpop_id = tpop3.id
        AND zj.jahr = (
          SELECT
            max(jahr)
          FROM
            zaehljahre
        WHERE
          zaehljahre.jahr <= tpop3.year
          AND zaehljahre.tpop_id = tpop3.id)
      ORDER BY
        tpop3.id,
        tpop3.year
),
...
FROM
  ...
  INNER JOIN apflora.ap_history ap4 ON ap4.id = pop4.ap_id
    ...
    AND pop4.year = tpop4.year ON tpop4.id = tplm.tpop_id
    AND tpop4.year = tplm.jahr
  WHERE
