...
const writeToStore = (store, data) => {
  runInAction(() => {
    data.forEach((d) => {
      d.beob_bereitgestellt = computed(() => store.table.beob_bereitgestellt.get(d.NO_NOTE_PROJET))
...
export default (store:Object, apArtId:number) => {
  ...
  app.db.beob_evab
    ...
    .then((data) => {
      ...
      recordValuesForWhichTableDataWasFetched({ store, table: `beob_evab`, field: `NO_ISFS`, value: apArtId })
    })
    .then(() => axios.get(url))
    .then(({ data }) => {
