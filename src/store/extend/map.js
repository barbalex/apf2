...
export default (store:Object) => {
  extendObservable(store.map, {
    ...
    mouseCoordEpsg21781: computed(
      () => {
        if (store.map.mouseCoord.length > 0) {
          return epsg4326to21781(store.map.mouseCoord[0], store.map.mouseCoord[1])
        }
        return []
      },
    ...
    setActiveBaseLayer: action(`setActiveBaseLayer`, (layer) => setActiveBaseLayer(store, layer)),
    overlays: observable([
    ...
    activeOverlaysSorted: computed(
      () => sortBy(store.map.activeOverlays, (activeOverlay) =>
        store.map.overlays.findIndex((overlay) => overlay.value === activeOverlay)
      ),
    ...
    addActiveOverlay: action(`addActiveOverlay`, layer => store.map.activeOverlays.push(layer)),
    removeActiveOverlay: action(`removeActiveOverlay`, (layer) => {
      store.map.activeOverlays = store.map.activeOverlays.filter(o => o !== layer)
    }),
    ...
    activeApfloraLayersSorted: computed(
      () => sortBy(store.map.activeApfloraLayers, (activeApfloraLayer) =>
        store.map.apfloraLayers.findIndex((apfloraLayer) =>
    ...
    removeActiveApfloraLayer: action(`removeActiveApfloraLayer`, (layer) => {
      store.map.activeApfloraLayers = store.map.activeApfloraLayers.filter(o => o !== layer)
    }),
