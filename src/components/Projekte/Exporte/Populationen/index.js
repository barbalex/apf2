            ...
            <DownloadCardButton
              ...
              onClick={async () => {
                ...
                const rows = (result?.data?.allPops?.nodes ?? []).map((z) => ({
                  ...
                  status:
                    z?.vPopVonapohnestatusesById?.nodes?.[0]?.status ?? '',
              ...
              onClick={async () => {
                ...
                const rows = (result?.data?.allPops?.nodes ?? []).map((z) => ({
                  ...
                  status_unklar_begruendung:
                    z?.vPopOhnekoordsById?.nodes?.[0]
                      ?.statusUnklarBegruendung ?? '',
                  lv95X: z?.vPopOhnekoordsById?.nodes?.[0]?.x ?? '',
                  lv95Y: z?.vPopOhnekoordsById?.nodes?.[0]?.y ?? '',
                  created_at:
                    z?.vPopOhnekoordsById?.nodes?.[0]?.createdAt ?? '',
                  updated_at:
                    z?.vPopOhnekoordsById?.nodes?.[0]?.updatedAt ?? '',
                  changed_by:
                    z?.vPopOhnekoordsById?.nodes?.[0]?.changedBy ?? '',
                }))
                removeNotification(notif)
                closeSnackbar(notif)
                if (rows.length === 0) {
